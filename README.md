...
```
본 프로젝트는 Kotra 내부 서비스 적용을 위해 현재 계약을 진행하였고, 코드를 포함한 최종 결과물의 사용 권한을 Kotra에 이관하였음을 알립니다.
...
**해외 관세청 세관고지를 신속하게 파악**하여 산업 관계자에게 제공하는 것을 목표로 한다. 이를 위해, Kotra(대한무역투자진흥공사)와 연계하여 각종 해결책 및 시스템 구성 방식을 구상하였다.

+ 해외 관세청에 업로드되는 세관 고지를 신속하게 파악 --> 매시간 세관고지를 크롤링하여 DB에 저장하는 **자동화 시스템** 구축  
...
## Data Set
### 1. 5개국 세관고지 데이터
각 나라별 관세청 사이트의 업로드된 세관고지 수집 진행 (날짜, 제목, 링크, 내용, 영어 번역본, 한국어 번역본)
|국가|기간|개수|
|:---:|:---:|:---:|
